...
class MiniMindVLM(MiniMindForCausalLM):
    ...
    def get_vision_model(model_path: str):
        ...
        # 如果本地路径不存在，尝试从 HuggingFace 下载
        if not os.path.exists(model_path):
            # 尝试使用 HuggingFace 模型名称
            if '/' in model_path or model_path.startswith('openai/'):
                # 看起来像是 HuggingFace 模型名称
            ...
            try:
                print(f"本地路径不存在，尝试从 HuggingFace 下载模型: {hf_model_name}")
                model = CLIPModel.from_pretrained(hf_model_name)
                processor = CLIPProcessor.from_pretrained(hf_model_name)
            except Exception as e:
                print(f"无法从 HuggingFace 下载模型: {e}")
                print(f"请确保网络连接正常，或者手动下载模型到: {model_path}")
                return None, None
        else:
            # 从本地路径加载
